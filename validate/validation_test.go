...
func TestIsValidPortNum(t *testing.T) {
	...
	for _, val := range goodValues {
		if err := IsValidPortNum(val); err != nil {
			t.Errorf("expected true for %d, got %v", val, err)
		}
	}
...
func TestIsInRange(t *testing.T) {
	...
	for _, val := range goodValues {
		if err := IsInRange(val.value, val.min, val.max); err != nil {
			t.Errorf("expected no errors for %#v, but got %v", val, err)
		}
	}
...
func TestIsValidGID(t *testing.T) {
	goodValues := createGroupIDs(0, 1, 1000, 65535, 2147483647)
	for _, val := range goodValues {
		if err := IsValidGID(val); err != nil {
			t.Errorf("expected true for '%d': %v", val, err)
		}
	}
...
func TestIsMapKeyName(t *testing.T) {
	successCases := []string{
		"a",
		"good",
		"good-good",
	...
	for i := range successCases {
		if errs := IsMapKeyName(successCases[i]); len(errs) != 0 {
	...
	for i := range failureCases {
		if errs := IsMapKeyName(failureCases[i]); len(errs) == 0 {
